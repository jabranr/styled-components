(function (global, factory) {
    ...
    ***************************************************************************** */
    ...
    var useInsertionEffectAlwaysWithSyncFallback = !isBrowser ? syncFallback : useInsertionEffect$1 || syncFallback;
    ...
    var ComponentStyle = /** @class */ (function () {
        ...
        ComponentStyle.prototype.generateAndInjectStyles = function (executionContext, styleSheet, stylis, insertionEffectBuffer) {
            var className = this.baseStyle
            ...
            if (this.isStatic && !stylis.hash) {
                if (this.staticRulesId && styleSheet.hasNameForId(this.componentId, this.staticRulesId)) {
                    ...
                    if (!styleSheet.hasNameForId(this.componentId, name_1)) {
                        ...
                        insertionEffectBuffer
                            ? insertionEffectBuffer.push([name_1, cssStaticFormatted])
                            : styleSheet.insertRules(this.componentId, name_1, cssStaticFormatted);
                    }
                    className = joinStrings(className, name_1);
                ...
                if (css) {
                    ...
                    if (!styleSheet.hasNameForId(this.componentId, name_2)) {
                        ...
                        insertionEffectBuffer
                            ? insertionEffectBuffer.push([name_2, cssFormatted])
                            : styleSheet.insertRules(this.componentId, name_2, cssFormatted);
                    }
                    className = joinStrings(className, name_2);
                }
            }
            return className;
        };
        ComponentStyle.prototype.flushStyles = function (buffer, styleSheet) {
            for (var i = 0; i < buffer.length; i++) {
                var _a = buffer[i], name_3 = _a[0], rules = _a[1];
                if (!styleSheet.hasNameForId(this.componentId, name_3)) {
                    styleSheet.insertRules(this.componentId, name_3, rules);
                }
            }
        };
    ...
    var useInsertionEffect = React['useInsertion' + 'Effect']
        ...
        : false;
    var shouldUseInsertionEffect = !IS_BROWSER ? false : Boolean(useInsertionEffect);
    ...
    function useInjectedStyle(componentStyle, resolvedAttrs) {
        var ssc = useStyleSheetContext();
        var insertionEffectBuffer = !ssc.styleSheet.server && shouldUseInsertionEffect && [];
        var className = componentStyle.generateAndInjectStyles(resolvedAttrs, ssc.styleSheet, ssc.stylis, insertionEffectBuffer);
